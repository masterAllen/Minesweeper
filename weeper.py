...
class Weeper:
            ...
            for j in range(table.shape[1]):
                if table[i, j].isdigit():
                    ...
                    if len(coordinates) > 0:
                        constraints[coordinates] = mine_count
                if table[i, j] == 'unknown':
                    unknown_coordinates.append((i, j))

        # 全局数量也有一个约束
        if self.mine_count < 10 and self.unknown_count < 20:
            constraints[Constraint(unknown_coordinates)] = self.mine_count
        for coordinates, mine_count in constraints.items():
            print(coordinates, mine_count)
        print('--------------------------------------------')
        ...
        while True:
            ...
            for now_coordinates, now_mine_count in constraints.items():
                for other_coordinates, other_mine_count in constraints.items():
                    # 如果不是子集，那跳过
                    if not now_coordinates.is_subset(other_coordinates):
                        continue

                    new_coordinates = other_coordinates - now_coordinates
                    new_mine_count = other_mine_count - now_mine_count

                    # 这里有可能 mine_count 是 0
                    if len(new_coordinates) > 0:
                        is_new = False
                        if new_coordinates in constraints:
                            if new_mine_count < constraints[new_coordinates]:
                                is_new = True
                        else:
                            is_new = True

                        if is_new:
                            new_constraints[new_coordinates] = new_mine_count

                    if new_mine_count < 0:
                        raise ValueError(f'new_mine_count < 0: {new_mine_count}')

            if len(new_constraints) == 0:
                break

            for new_coordinates, new_mine_count in new_constraints.items():
                constraints[new_coordinates] = new_mine_count
    ...
    def solve_by_ensure(self, constraints: dict) -> tuple[set, set]:
        ...
        for coordinates, mine_count in constraints.items():
            ...
            # 如果小于，那么报错
            if len(coordinates) < mine_count:
                raise ValueError(f'len(coordinates) < mine_count: {len(coordinates)} < {mine_count}')
            if mine_count < 0:
    ...
    def solve_by_force(self, constraints: dict):
        ...
        for coordinates, mine_count in constraints.items():
            if len(coordinates) < 10:
                score = math.comb(len(coordinates), mine_count)
                scores.append(score)
        ...
        for i in range(len(coordinates_list)):
            ...
            mine_count = constraints[coordinates]

            table_bak = self.table.copy()

            # 从 coordinates 中随机选择 mine_count 个坐标，设置为 mine
            tables = []
            for marked_mine_coordinates in itertools.combinations(coordinates, mine_count):
