'''
[E2]: 十字差，线索表示周围两格范围内的纵向雷数减去横向雷数的差值
'''
import numpy as np
import utils
    ...
    for i in range(table.shape[0]):
        for j in range(table.shape[1]):
            ...
            if n is not None:
                ...
                # v - h = n，所以 v = h + n，h = v - n
                v_min2 = h_min_value + n
                v_max2 = h_max_value + n
                vertical_ranges[key] = _merge_range(vertical_ranges.get(key), (v_min2, v_max2))

                h_min2 = v_min_value - n
                h_max2 = v_max_value - n
                horizontal_ranges[key] = _merge_range(horizontal_ranges.get(key), (h_min2, h_max2))

                # print(f'center = {(i, j)}, v_min_value = {v_min_value}, v_max_value = {v_max_value}, h_min_value = {h_min_value}, h_max_value = {h_max_value}')
    ...
    for i in range(table.shape[0]):
        for j in range(table.shape[1]):
            ...
            if n is not None:
                ...
                # 获取当前格子的横纵范围
                h_min_value, h_max_value = horizontal_ranges.get(key, (1, 100))
                v_min_value, v_max_value = vertical_ranges.get(key, (1, 100))

                # 更新横纵方向的情况：v - h = n
                new_v_min = max(h_min_value + n, v_min_value)
                new_v_max = min(h_max_value + n, v_max_value)
                vertical_ranges[key] = (new_v_min, new_v_max)

                new_h_min = max(v_min_value - n, h_min_value)
                new_h_max = min(v_max_value - n, h_max_value)
    ...
    for i in range(table.shape[0]):
        for j in range(table.shape[1]):
            ...
            if n is not None:
                ...
                if n < (v_min_value - h_max_value) or n > (v_max_value - h_min_value):
                    return False
    return True
